...
export const useVentasPaginadas = (startDate, currentPage, searchTerm = "") => {
	const queryClient = useQueryClient();

	// Obtener ventas paginadas con búsqueda opcional
	const { data, isLoading, isError } = useQuery({
		queryKey: ["ventasPaginadas", startDate, currentPage, searchTerm],
		queryFn: async () => {
			const { data } = await Axios.get("/venta", {
				params: {
					...
					page: currentPage,
					search: searchTerm.trim(), // Incluir término de búsqueda si existe
				},
			});
			return data;
		},
	...
	const eliminarVenta = useMutation({
		...
		onSuccess: () => {
			queryClient.invalidateQueries(["ventasPaginadas", startDate, currentPage, searchTerm]); // Refrescar ventas
		},
	});
